...
The notice command preserves the base 367-package text byte-for-byte in a default build. In a verified opt-in source build, it appends an `OPTIONAL OCR ENGINE LICENSES` section containing those three exact sidecar texts in the table order. It reads license texts only; it does not execute the engine. If an optional text is missing, tampered, oversized, non-UTF-8, or path-unsafe, it returns `Optional OCR license texts could not be verified.` before the intact base text and does not expose resource paths, hashes, or identities. A focused license-only fixture produced the successful append without an engine file; that fixture does not establish which files a complete installer contains. A retained opt-in bundled-resource readback produced a 24,473-byte appended section containing the three verified sidecar texts; its final warning-order log is `target/ocr-page-probe/logs/notices-retained-readback-warning-final-v2-20260925.log`, SHA-256 `71E67C086942CB88D628D7A874CA8102D7072801B6357AC54C2AB3F9F05A57AF`. Its resource inventory is `target/ocr-notices-proof-20260925/resource-inventory.json`, SHA-256 `6F37F252AE4A2F8717168DD17EF2923CC8DCC75AAA60F6ECDE20D3E41F4775BA`. The existing dialog displays the returned string; frontend mocks verify the default, ordered opt-in, and warning cases, but not native desktop or installed-app interaction.

Any future OCR distribution needs to keep this separate inventory and its copied source texts accurate, then decide and verify how users reach them. The signed-OCR installer path fails closed before an output root, credentials, build, or signing; its retained control log is `target/ocr-page-probe/logs/installer-signed-ocr-refusal-20260925.log`, SHA-256 `E5C0794710A39270F47D58AFE6503427E7489A523DA74A5F0D9309B334CAF048`. Azure-signed OCR packaging remains unsupported pending a signed-engine/Tauri hash-order solution. This is an inventory and provenance record, not a complete licensing review or legal advice.
